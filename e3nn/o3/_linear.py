...
from e3nn.adapter_tool import calc
from icecream import ic
import os


init_std = os.environ.get('MMEA_STD', None)
init_rank = os.environ.get('MMEA_RANK', 16)
if init_std:
...
class Linear(CodeGenMixin, torch.nn.Module):
        ...
        *,
        ...
        if internal_weights and self.weight_numel > 0:
            ...
            for ins in self.instructions:
                ic(ins)
                if ins.i_in != -1 and ins.i_in != 0:
                    path_in.append(ins.path_shape[0])
                    path_out.append(ins.path_shape[1])
                    n_ins += 1

            self.adapter_tensor_head = torch.nn.Sequential(
                torch.nn.Linear(self.instructions[0].path_shape[0], self.r),
                torch.nn.SiLU(),
            )
            self.adapter_scale = torch.nn.Sequential(
                torch.nn.Linear(self.r, sum(path_out)),
            )

            self.adapter_scalar = torch.nn.Sequential(
                torch.nn.Linear(self.r, self.instructions[0].path_shape[1]),
            )

            

            with torch.no_grad():
                self.adapter_tensor_head[0].bias.zero_()
                self.adapter_scale[0].weight.zero_()
                self.adapter_scale[0].bias.zero_()
                self.adapter_scalar[0].bias.zero_()
                # self.adapter_scalar[0].weight.zero_()
                if init_std is None:
                    pass
                elif init_std > 0:
                    torch.nn.init.normal_(self.adapter_scalar[0].weight, mean=0, std=init_std)
                else:
                    self.adapter_scalar[0].weight.zero_()

            ic(self.adapter_tensor_head)
            ic(self.adapter_scale)
            ic(self.adapter_scalar)
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.irreps_in} -> {self.irreps_out} | {self.weight_numel} weights | adapter)"
    ...
    def forward(self, features, weight: Optional[torch.Tensor] = None, bias: Optional[torch.Tensor] = None):
        ...
        if bias is None:
            ...
            bias = self.bias
        ori_out = self._compiled_main(features, weight, bias)
        
        modulated_out = calc(features=features, ori_out=ori_out, irreps_in=self.irreps_in, irreps_out=self.irreps_out, adapter_tensor_head=self.adapter_tensor_head, adapter_scalar=self.adapter_scalar, adapter_scale=self.adapter_scale)
...
if __name__ == "__main__":
    ic.enable()
    linear = Linear("128x0e+128x1o", "128x0e+128x1o")
