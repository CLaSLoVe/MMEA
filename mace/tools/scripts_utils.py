    ...
    for name in sorted(all_data_loaders, key=custom_key):
        ...
        if table_type == "TotalRMSE":
            table.add_row(
                [
                    name,
                    f"{metrics['rmse_e'] * 1000:.4f}",
                    f"{metrics['rmse_f'] * 1000:.4f}",
                    f"{metrics['rel_rmse_f']:.4f}",
                ]
            )
        elif table_type == "PerAtomRMSE":
            table.add_row(
                [
                    name,
                    f"{metrics['rmse_e_per_atom'] * 1000:.4f}",
                    f"{metrics['rmse_f'] * 1000:.4f}",
                    f"{metrics['rel_rmse_f']:.4f}",
                ]
            )
            ...
            table.add_row(
                [
                    ...
                    f"{metrics['rmse_f'] * 1000:.4f}",
                    f"{metrics['rel_rmse_f']:.4f}",
                    f"{metrics['rmse_virials'] * 1000:.4f}",
                ]
            )
        elif table_type == "TotalMAE":
            table.add_row(
                [
                    name,
                    f"{metrics['mae_e'] * 1000:.4f}",
                    f"{metrics['mae_f'] * 1000:.4f}",
                    f"{metrics['rel_mae_f']:.4f}",
                ]
            )
        elif table_type == "PerAtomMAE":
            table.add_row(
                [
                    name,
                    f"{metrics['mae_e_per_atom'] * 1000:.4f}",
                    f"{metrics['mae_f'] * 1000:.4f}",
                    f"{metrics['rel_mae_f']:.4f}",
                ]
            )
            ...
            table.add_row(
                [
                    name,
                    f"{metrics['rmse_mu_per_atom'] * 1000:.4f}",
                    f"{metrics['rel_rmse_mu']:.4f}",
                ]
            )
        elif table_type == "DipoleMAE":
            table.add_row(
                [
                    name,
                    f"{metrics['mae_mu_per_atom'] * 1000:.4f}",
                    f"{metrics['rel_mae_mu']:.4f}",
                ]
            )
        elif table_type == "EnergyDipoleRMSE":
            table.add_row(
