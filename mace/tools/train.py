...
def valid_err_log(valid_loss, eval_metrics, logger, log_errors, epoch=None):
    ...
    if log_errors == "PerAtomRMSE":
        ...
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.4f} meV, RMSE_F={error_f:.4f} meV / A"
        )
    elif (
        ...
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.4f} meV, RMSE_F={error_f:.4f} meV / A, RMSE_stress_per_atom={error_stress:.4f} meV / A^3"
        )
    elif (
        log_errors == "PerAtomRMSEstressvirials"
        ...
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.4f} meV, RMSE_F={error_f:.4f} meV / A, RMSE_virials_per_atom={error_virials:.4f} meV"
        )
    elif log_errors == "TotalRMSE":
        error_e = eval_metrics["rmse_e"] * 1e3
        error_f = eval_metrics["rmse_f"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E={error_e:.4f} meV, RMSE_F={error_f:.4f} meV / A"
        )
    elif log_errors == "PerAtomMAE":
        error_e = eval_metrics["mae_e_per_atom"] * 1e3
        error_f = eval_metrics["mae_f"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, MAE_E_per_atom={error_e:.4f} meV, MAE_F={error_f:.4f} meV / A"
        )
    elif log_errors == "TotalMAE":
        error_e = eval_metrics["mae_e"] * 1e3
        error_f = eval_metrics["mae_f"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, MAE_E={error_e:.4f} meV, MAE_F={error_f:.4f} meV / A"
        )
    elif log_errors == "DipoleRMSE":
        error_mu = eval_metrics["rmse_mu_per_atom"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_MU_per_atom={error_mu:.4f} mDebye"
        )
        ...
        error_mu = eval_metrics["rmse_mu_per_atom"] * 1e3
        logging.info(
            f"Epoch {epoch}: loss={valid_loss:.4f}, RMSE_E_per_atom={error_e:.4f} meV, RMSE_F={error_f:.4f} meV / A, RMSE_Mu_per_atom={error_mu:.4f} mDebye"
        )
...
def take_step(
    ...
    for name, param in model.named_parameters():
        if "LoRA" in name or "adapter" in name or "radial_embedding" in name or ("symmetric_contractions" in name and "weights_max" not in name):
