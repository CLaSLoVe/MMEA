...
def run(args: argparse.Namespace) -> None:
    ...
    def init_adapter_scalar(m: torch.nn.Module, method, std=1e-3):
        if hasattr(m, "adapter_scalar"):
            w = m.adapter_scalar[0].weight
            if method == "zero":
                w.zero_()
            else:
                torch.nn.init.normal_(w, mean=0.0, std=std)

    if args.mmea_init == "zero":        
        init_adapter_scalar(model, "zero")
    elif args.mmea_init == "normal":
        init_adapter_scalar(model, "normal", std=args.mmea_std)
    elif args.mmea_init is None:
        pass
